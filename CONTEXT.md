...
- **컴플라이언스**(ISMS-P·ISO 27001) — 자동판정 통제는 4개다(`patch`/`asset`/`secops` +
  #546 이 올린 `account`). 계정 인벤토리가 제품 안에 생기면서 증적이 DB 에 있게 됐고,
  체크리스트에는 증적이 **제품 밖**에 있는 4건(정책문서·접근권한 검토·사고대응·재해복구)이
  남는다(`access_review` 는 접속기록 점검 기능 제거로 자동판정 근거가 사라져 체크리스트로 내려갔다). `patch` 는 통제 전체가 아니라 **버킷(KEV/CRITICAL/HIGH)별로** 판정한다 — 이력이 짧아
...
- **조치 상태**(#582) — 같은 자연키에 미조치·조치중·완료·예외와 메모 한 줄만 둔다. 예외는 자동
  억제가 아니며 완료·예외는 SLA 남은 일수에서 제외한다. 담당자·결재선·예외 만료는 만들지 않는다.
- **Export API** — `GET /export.php`(JSON/XML). 웹 로그인 세션과 `assets` 권한으로 인증한다.
  폐지된 전용 API 토큰은 쓰지 않는다. 상세: `docs/dev/export-api.md`.
- **SBOM 산출 API**(#554) — `GET /sbom.php`(CycloneDX 1.5 / SPDX 2.3, `src/purl.php` 가 purl 생성).
  인증은 export.php 와 같은 웹 로그인 세션이고, **자산 하나당 문서 하나**다(여러 호스트를 한 문서에
...
- **전이 의존성 판정**(#560) + **부모별 조치 묶음**(#565) — 그래프를 조치로 잇는다. 취약점 행이
  전이면 조치 문구를 "**직접 조치 불가 — X 가 끌어옴**"으로 바꾸고(그 패키지만 갈아끼우면 부모가
  깨진다), 부모별로 묶어 "이 하나를 올리면 N건 해결"을 취약점 탭 상단에 **최고 심각도 → 건수**
  순으로 올린다. 그래프에 없는 행(`unknown`)은 아무것도 표시하지 않고(엣지 없는 자산이 다수라
  화면이 "모름"으로 도배된다), 집계는 **스캔 전체** 기준이다(페이지마다 답이 달라지는 우선순위는
  우선순위가 아니다). **올릴 버전은 제안하지 않는다** — 설치되지 않은 부모 버전이 무엇을 끌어오는지
  수집물로 알 수 없다. 위 인덱스 제약 때문에 `host.php` 에만 붙고 전역 목록에는 없다.
- **접속기록 5요소**(#496) · **세션·에이전트 토큰 유효기간**(#492) · **Caddy 보안 응답 헤더**(#497,
  HSTS 는 자체서명 확정이라 붙이지 않는다) — 상세는 `docs/dev/architecture.md §6`·`docs/ui-configuration.md`.
- **제품 범위 정리** — 내부 담당자·결재선·예외 만료를 추적하는 조치 관리 기능은 제거했다. 상태 4종과
  메모 한 줄만 유지하고 알림은 만들지 않는다. 제품은 스캔 결과와 판정 근거를 정확히 보여주는 데 집중한다.
