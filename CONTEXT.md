...
> 현행 기준: 2026-08-21 · 에이전트 3.23 · 자산 탐색(섀도우 IT)·세그먼트 맵·기반시설 U-코드
...
- **읽기 전용**, 서버에 무리 안 감(nice 19 / ionice idle / 명령별 timeout). 피크 메모리 실측 **61.6MB**
  (Debian 12 · 91패키지 — 페이로드 크기에 비례. 수치·재측정법은 `docs/dev/에이전트-리소스-프로파일.md`)
- jq 있으면 JSON, 없으면 섹션 텍스트로 폴백. RHEL/Debian 자동 감지
- 설치는 `sudo bash install-agent.sh` 하나 — systemd 있으면 상시 데몬(10초 poll), 없으면 cron 폴백
...
- 쉘 에이전트(상시 데몬, 10초 poll) → Caddy(HTTPS:8080) → `ingest.php` → MySQL 순으로 사실을 적재
  (중앙 서버 자신의 로컬 에이전트만 예외로 `web:8081` 루프백 평문 사용)
- 적재된 사실을 로컬 CVE 미러(NVD·OSV·KISA) 매칭 + 런타임 노출 상관 + EPSS/KEV 가중으로 판정해
  PHP 웹 대시보드가 보여준다

그림과 파이프라인 분기는 [`docs/dev/architecture.md` §1](docs/dev/architecture.md).

---

## 7. 매처 핵심 규칙 (구현됨)

수집한 `packages` + `exposures`(포트) + `processes`(실행/로드)를 CVE와 조인해 런타임 노출 상태를
7단계로 판정한다(`vg_classify`).

| 등급 | 상태 | 비고 |
|---|---|---|
| 3 / HIGH | `EXTERNAL`(외부 노출) | **KEV 등재** 시 **CRITICAL** 로 상향 |
| 2 / MEDIUM | `LAN`·`FILTERED`·`LISTENING`·`RUNNING`·`LOADED` | EPSS·CVSS 는 같은 등급 안 정렬에만 사용 |
| 1 / LOW | `INSTALLED`(설치만 됨) | — |

`FILTERED` 판정이 없으면 방화벽 뒤 내부 서비스가 **전부 HIGH/CRITICAL 로 뜬다**(오탐).

**오탐 억제**는 데비안 중심의 4겹(①OSV 버전필터 ②changelog ③errata ④debsecan 역방향)이고, RHEL
계열·우분투·커널은 각자의 벤더 소스로 별도 판정한다.

- 억제된 건은 `tb_finding` 이 아니라 `tb_suppressed_finding` 으로 가 위험 집계에서만 빠지고, **근거는
  화면에 그대로 남는다**(숨기지 않는다)
- 벤더가 "아직 안 고쳤다"고 확인한 CVE 는 `tb_finding.no_fix` — 등급은 유지하고 "지금 고칠 수 있는
  것"과 화면에서만 분리한다
- **억제를 취소하는 두 신호**: 패치됐어도 프로세스가 옛 `.so` 를 물고 있거나(`tb_stale_lib`, 조치는
  재시작) 커널이 재부팅 전이면 억제하지 않는다 — 없으면 "패치됨=안전"으로 착각해 미탐이 난다
...
**보안설정 점검(CCE)** 은 같은 수집물을 다른 눈으로 본다 — `src/cce.php` 가 **39개 항목**으로 판정하고,
그 결과가 어느 기준의 증적인지는 `tb_control_mapping` 이, 기준 전체 중 몇 개를 덮는지는
`tb_control_catalog`(U-코드 72개)가 분모로 답한다(SSOT, 역할이 다른 두 표). **계정 인벤토리**도 같은
원칙 — 못 읽은 항목은 NA, 공유·퇴직자 계정 추정은 FAIL 이 아니라 REVIEW다.
...
- **기본 인프라 완료**: Docker compose(dev/prod) + Docker Secrets + 러너 · 수집→전송→저장 · 로그인~자산
  상세~취약점 화면 · 설정형 RBAC(admin 은 코드에서 항상 허용해 잠금을 막는다) · Caddy HTTPS · 무중단
  배포(prod 가 `../server` 를 ro 마운트) · 접속기록 5요소 · 세션·토큰 유효기간 · 보안 응답 헤더
- **스키마 마이그레이션 자동화** — `deploy/migrate.sh` 가 미적용분만 사전순으로 적용하고
  `tb_schema_migrations` 에 기록한다. 파일명은 **타임스탬프**(연번은 동시 브랜치가 같은 번호를 집어
  실제로 중복이 생긴 이력이 있어 pre-push 가 막는다). DB 규약은 `docs/dev/데이터베이스.md`
- **에이전트 운용** — 상시 데몬(10초 poll)이라 즉시 실행·예약·주기 변경을 중앙 웹에서 한다(SSH 재설치
  불필요). 설치 파일은 배포별 루트 CA 로 서명해 배포하고, 호스트별 자원 상한은 속도 티어가 내려보낸다
- **무인 자동 업데이트 + 커밋 시점 Ed25519 서명 검증** — 서명은 유지보수자가 커밋 전 로컬 개인키로
  만들고(`deploy/agent_sign.sh`) 웹은 파일을 읽어 전달만 한다 — **웹 티어가 뚫려도 위조가 안 된다.**
  검증 실패 시 자동 업데이트만 건너뛰고(수집·poll 은 계속), 다운그레이드는 거부하며 감사에 남는다

### 판정 — 매칭·억제·벤더

| 항목 | 왜 이렇게 했나 |
|---|---|
| **피드 커넥터 11종 가동**(KEV/OSV/NVD/KISA/EPSS + 벤더별 6종) + 범용 API | 역할은 `docs/dev/피드소스-역할.md`. pkgregistry 는 12번째 고정 타입이지만 시드된 인스턴스가 없어 가동 수엔 안 잡힌다 |
| **NVD 전체 데이터**(약 36만 건) | 주기 수집을 수정일(lastMod) 기준으로 함 — 발행일 기준이면 뒤늦게 CVSS 가 붙는 CVE 를 영원히 놓친다. 전체 백필은 `bin/backfill_nvd.php` |
| **패키지 출처 판정** | dpkg 는 vendor 를 안 줘 apt 라벨로 서드파티를 가린다(URL 기준이면 사내 미러가 오분류된다) |
| **재매칭 지문**(`tb_scan.match_fingerprint`) | 결과가 같으면 트랜잭션조차 안 연다 — 예전엔 1비트도 안 바뀐 경우까지 삭제·재삽입해 binlog 가 하루 20GB 쌓였다. 판정 로직을 바꾸면 `VG_MATCH_FP_VERSION` 을 올린다 |
| **판정 근거 구조화**(`tb_finding_evidence` + `tb_collection_stage`) | 한 단계가 비면 그 영역이 조용히 "취약점 없음"이 되므로 자산 상세에 경고로 띄운다 |

### 화면·기능

> 화면마다 "왜 이렇게 보여주나"(판정 조건·걷어낸 요소·믿을 수 있는 범위)는
> **[`docs/dev/화면-안내.md`](docs/dev/화면-안내.md)** 가 갖는다. 여기는 무엇이 있는지와 핵심 설계
> 판단만 적는다.

| 기능 | 핵심 설계 판단 |
|---|---|
| **자산 탐색**(섀도우 IT) | 관리 대역(CIDR)의 미설치 IP 를 2단계 TCP connect 스윕으로 찾아 대조. 웹은 스캔을 돌리지 않고 pending 만 넣으면 스케줄러 틱이 집행 |
| **세그먼트 맵** | 라우팅 테이블로 대역별 카드를 그린다. 실제 트래픽 엣지는 수집 데이터가 없어 그리지 않는다(없는 것을 채우면 틀린 그림이 사실처럼 보인다) |
| **기반시설 U-코드 커버리지** | `tb_control_catalog` 72개를 분모로 왼쪽 조인해 덮지 않는 항목까지 드러낸다 |
| **컨테이너·SBOM** | 실행 중 컨테이너 rootfs 를 직접 읽어(docker CLI 비의존) 내부 패키지 수집. `GET /sbom.php` 로 CycloneDX 1.5/SPDX 2.3 산출(자산당 문서 1개, 결정적 UUIDv5) |
| **의존성 그래프**(`depgraph.php`) | 전이 의존성이면 조치를 "직접 조치 불가 — X 가 끌어옴"으로 부모별 그룹화. 올릴 버전은 제안하지 않는다 |
| **탐지 결과 한 화면**(`findings.php`) | CVE·CCE·노출을 `?type=` 탭으로 묶되 세 표를 UNION 하지 않는다(합쳐 정렬·페이징하면 인덱스가 죽는다) |
| **컴플라이언스**(`src/compliance.php`) | 자동판정 통제 4종(patch/asset/secops/account), 판정 어휘 4종(준수/부분준수/미준수/**판정 불가**) — 0건을 준수로 쓰면 심사 증빙이 허위 안심이 된다. 증적이 제품 밖인 4건은 수동 체크리스트로 강등(삭제 아님) |
| **자산 중요도·N2SF 등급** | 확정값·제안값을 다른 컬럼에 담는다(등급 확정은 기관의 법적 처분이라 시스템이 대신할 수 없다). 제안 이력은 append-only |
| **성능 사전집계** | `tb_package_summary`(92만 행 GROUP BY 8초 → 0.1초 미만) 등. 라이선스 요약은 OSV 게이트에 묶지 않는다(묶으면 OSV 미등록 동안 KPI 가 영구히 0으로 보인다) |
| **화면 전면 재구성** | 목록·표 위주에서 카드·도넛·랭킹 차트로 전환(Chart.js 로컬 렌더링·디자인 토큰 통일). 기준은 `docs/dev/ui-design-system.md` |
| **그 밖의 읽는 화면** | 변화 추적(`changes.php`) · 벤더 판정 조회 · SSG 룰 카탈로그 · 제거·대체 권고(EOL 확정 아님) · 조치 상태 4종 · 계정 인벤토리 · 패키지 무결성 · Export API |
| **제품 범위 정리** | 담당자·결재선·예외만료 추적형 조치관리, 접속기록 월간검토, 정책·절차 문서 심사 카드는 제거·강등 — 제품은 스캔 결과와 판정 근거에 집중 |
