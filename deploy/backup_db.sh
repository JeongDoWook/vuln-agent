...
#
# ⚠ 버퍼 풀은 곧 RAM 이다. 저장 위치가 tmpfs 면 이 컨테이너가 RAM 을 5.6GB 까지 점유한다
# (#620 이 데이터를 디스크로 옮겨 685MB 로 내려왔다). 운영 호스트 기준 가용 RAM 은 15GiB 중
# 2.4GiB 규모다 — **이 값을 올리면 호스트 RAM 을 그만큼 더 먹는다.** 512M 는
...
verify_restore() {
  ...
  #
  # /var/lib/mysql 은 **익명 볼륨(디스크)** 이다. tmpfs(=RAM)는 DB 가 작을 때만 성립한다 —
  # 5.27GB DB 를 복원하면 컨테이너가 그만큼의 RAM 을 점유하고(가용 4.7GB·swap 0 이면 고갈),
  # tmpfs 에 상한을 두면 이번엔 복원이 'table is full' 로 실패한다. 상한을 올리는 건 답이
  # 아니다(그만큼의 RAM 이 애초에 없다). 같은 크기를 디스크로는 여유 있게 받는다.
  #
