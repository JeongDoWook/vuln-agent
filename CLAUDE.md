...
- **스모크는 "스택이 내 트리를 서빙 중일 때"만 돈다.** 스택이 하나라 "떠 있다" 는 것만으로는
  ...
  내 트리로 가져오려면 그 트리에서 `dev up -d`. 일부러 다른 대상을 칠 때만 `VG_SMOKE_ANY=1`.
...
구현을 맡기고, 결과를 취합해 이 창에 보고한다. 메인은 편집·시행착오 컨텍스트를 안 짊어져 가볍게
유지된다. Windows PowerShell 전용, tmux 불필요. (자세한 건 `deploy/orchestrator/README.md`.)

- **흐름**:
  1. 메인: 관련 코드를 조사하고 계획을 세운다(어느 파일·무슨 변경·어떻게 검증). 계획을 한두 줄로
     알린 뒤 진행한다(막지 않음. 규모가 크거나 되돌리기 어려우면 먼저 확인).
  2. 메인: 하위작업마다 `.\deploy\orchestrator\spawn-worker.ps1 -Task <슬러그> -Prompt "<구체적 구현 지시>"`
     로 워커를 띄운다. **독립 하위작업이 여럿이면 여러 워커로 나눠 병렬로**, 하나면 워커 하나로.
...
- **왜**: 메인 컨텍스트가 편집으로 불어나 `/compact` 를 부르는 걸 막는다 — 무거운 건 워커 탭에.
