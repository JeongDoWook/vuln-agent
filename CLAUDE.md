...
- 커넥터는 고정 11종(KEV/OSV/NVD/KISA/EPSS와 벤더·커널·SSG 피드) 및 실제 사용자 정의 범용 API만 지원한다. 가상의 소스를 위한 추상화는 실제 요구가 생길 때 추가.
...
## 보안·운영 준수 원칙 (조직 표준 — 항상 확인)
새 기능·화면·API 를 만들 때 아래도 YAGNI/KISS/DRY/SOLID 와 함께 확인한다.
이미 있는 대응 수단부터 재사용하고, 없으면 아래 패턴으로 만든다.

1. **하드코딩 금지 / 설정으로 관리** — URL·경로·임계값·만료시간·페이지크기 등 매직 넘버는
   설정·환경변수·DB 로 뺀다. 비밀값 → `secrets/*.txt`(Docker Secrets), NVD API 키 등은 DB 저장
   (코드/저장소에 평문 없음), 포트·DB 접속정보 → `deploy/.env.*`.
   예외: 고정 5종 피드(kev/osv/nvd/kisa/epss)의 스키마 매핑처럼 "바뀔 일이 없는 알려진 구조"는
   KISS/YAGNI 상 하드코딩을 허용한다 — 범용 설정으로 감싸는 대신 새 피드는 `VgFeedConnector`
   구현으로 확장한다(SOLID·OCP 와 동일 원칙).
2. **공통 로직 모듈화** — 새로 짜기 전에 `server/src/` 기존 헬퍼부터 찾는다: `vg_h()`(XSS 이스케이프),
   `vg_pdo()`, `vg_secret()`, `vg_header/footer()`, `vg_upsert_*()`, `vg_page_nav()`(페이지네이션),
   `vg_toolbar()`/`vg_table()`(검색·목록 렌더), `vg_require_menu()`(RBAC 서버측 검증),
   `vg_log_activity()`(감사로그), `vg_soft_delete()`. 2회 이상 반복되면 헬퍼로 뽑는다(DRY 와 동일 기준).
3. **시큐어코딩** — SQL 은 PDO prepared statement 만 쓴다(문자열 concat 쿼리 금지). 사용자 입력을
   포함한 출력은 반드시 `vg_h()` 로 이스케이프. 인가는 클라이언트 숨김이 아니라 서버측
   `vg_require_menu()`/역할 검사로 확정한다. 에러 화면·API 응답에 예외 원문(스택트레이스·SQL·경로)을
   노출하지 않는다 — 사용자에겐 일반화된 메시지, 상세는 서버 로그로만.
4. **중요데이터 암호화** — 비밀번호는 `password_hash()`(복호화 불가 해시). API 토큰은 DB 에 해시만
   저장하고 원문은 발급 시 1회만 노출. 시크릿·자격증명은 Docker Secrets 로만 관리(코드/DB 평문 금지).
   전송구간은 Caddy 의 TLS 를 전제로 한다. 새로 저장하는 값이 인증정보·토큰·개인정보류면 커밋 전
   평문 저장 여부를 반드시 확인한다.
5. **시스템 연계** — 외부 연동(피드 커넥터, 에이전트→`ingest.php`)은 실패·타임아웃·재시도를 갖춘다
   (예: `http.php` 의 타임아웃 상수). 커넥터는 `VgFeedConnector.run(PDO,$conn): {fetched,upserted}`
   계약을 지켜 연계 규격을 코드로 고정한다. 새 연동도 같은 실패처리 패턴을 따른다.
6. **성능/응답속도** — 목록·통계 화면은 인덱스 + 페이지네이션(`vg_page_nav`)을 기본으로 하고,
   대량 집계는 사전집계 테이블(예: `tb_package_summary`)을 검토한다. N+1 이 되는 조인은 피하고,
   무거운 작업(전체 재매칭·백필 등)은 `bin/`의 CLI/스케줄러로 분리해 웹 요청을 막지 않는다.
7. **로그 관리** — 사용자의 접근·조작(로그인/로그인 실패/조회·열람/export/생성·수정·삭제)은
   `vg_log_activity()` 로 `tb_activity_log` 에 남긴다. 새 화면·API 를 추가할 때 "이것도 감사로그
   대상인가"를 먼저 판단한다 — 특히 상세조회·export 처럼 누가 뭘 봤는지가 중요한 열람 행위.
   로그에 비밀번호·토큰 원문 등 민감정보를 남기지 않는다.
8. **검색·열람(분류 기준)** — 목록 화면의 필터 기준(심각도·상태·KEV·연도 등)은 DB 값/상수로
   관리하고 하드코딩된 분류표를 늘리지 않는다. 대량 데이터 목록·검색엔 인덱스+페이징을 기본으로
   붙이고(원칙 6과 연계), 열람 자체도 인가 검증 + 감사로그 대상이다(원칙 3·7과 연계).
...
5. 매직 넘버·URL·경로를 코드에 박지 않았나? (하드코딩 금지)
6. SQL 은 prepared statement 인가, 출력은 `vg_h()` 로 이스케이프했나, 인가는 서버측에서 확정하나?
7. 새로 저장하는 값이 비밀번호·토큰·개인정보면 해시/Docker Secrets 로 가는가, 평문이 남지 않는가?
8. 이 화면·API 는 사용자의 접근/열람/조작을 `vg_log_activity()` 로 남기는가?
9. 목록·검색 화면에 인덱스·페이지네이션이 붙어 있는가?
