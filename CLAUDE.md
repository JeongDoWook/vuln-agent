...
- **그래도 스택은 서버 코드를 건드릴 때만 올린다.** 워크트리를 팠다고 반사적으로 `up -d` 하지 않는다.
  - `agent/**`·`docs/**`·`*.md` 만 바꿨다 → **스택 불필요.** `bash -n` + 실제 컨테이너 e2e 로 검증한다
    (스택을 띄워도 검사되는 게 없다).
  - `server/**`·`db/**`·`tests/**` 를 바꿨다 → 이미 떠 있는 이 트리 전용 스택 그대로 **스모크만**
    돌린다. **에이전트는 `dev up -d` 를 실행하지 않는다** — 스택 기동/전환은 사용자가 직접 한다
    (`.claude/hooks/block-dev-stack.sh` 가 코드로도 막는다 — 컨테이너명이 트리마다 고유해진
    뒤에도 "컨테이너 기동/중지는 사람이 한다"는 원칙 자체는 그대로 유지한다).
    이 트리 전용 컨테이너가 안 떠 있어 스모크가 중단(exit 2)되면, 그대로 사용자에게 보고하고 기다린다.
- **스모크는 "이 트리 전용 컨테이너가 떠 있을 때"만 돈다.** `tests/smoke.sh` 와 pre-push 훅은
  ...
  **에이전트는 이때도 `dev up -d` 를 실행하지 않는다** — 사용자에게 알리고 기다린다.
...
#   이 워크트리 전용 web/scheduler 스택이 이미 떠 있으면 그대로 스모크만 돌린다.
./tests/smoke.sh http://localhost:<wt.sh add 가 알려준 포트>
#   이 워크트리 전용 컨테이너(vulnagent-web-dev-무엇)가 떠 있으면 통과. 안 떠 있으면 스모크가
#   스스로 중단한다(exit 2) — 그러면 사용자에게 보고하고 기다린다(에이전트가 up -d 를 대신 치지 않음).

# 구현 → 검증 게이트 → 커밋 → push → PR
./deploy/wt.sh rm 무엇             # 워크트리 제거(이 워크트리 전용 스택만 대상 — 공용 DB 는 안 건드림)
```
...
- pre-push 훅은 **이 트리 전용 컨테이너가 떠 있을 때만** 스모크한다(컨테이너명이 워크트리마다
  ...
  `tests/smoke.sh` 도 같은 확인을 하고, 안 떠 있으면 **중단**한다(exit 2).
...
- **에이전트는 `./deploy/compose_runner.sh dev up -d`/`dev down` 을 실행하지 않는다** — web+scheduler
  가 워크트리별 독립 컨테이너로 떠서(컨테이너명이 트리마다 고유) "서로 스택을 뺏는다"는 옛 이유는
  없어졌지만, 컨테이너 기동·중지는 항상 사용자가 명시적으로 한다는 원칙 자체는 그대로다.
  이 트리 전용 컨테이너가 안 떠 있어 스모크가 중단(exit 2)되면, 에이전트는 결과를 그대로 보고하고
  기다린다. **문서뿐 아니라 `.claude/hooks/block-dev-stack.sh` 가 코드로도 막는다**(과거엔 문서만
  있어서 여러 세션이 무시하고 직접 쳤다).
