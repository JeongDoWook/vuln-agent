...
- **흐름**:
  ...
  2. 메인: 하위작업마다 워커를 띄운다. **2개 이상으로 쪼개는 순간부터는 인라인 `-Prompt "..."`
     로 그 자리에서 조립하지 말고, 지시문을 먼저 파일로 써 둔다** — `.omc/tasks/<슬러그>.md`
     하나당 하위작업 하나(번호가 아니라 내용을 알 수 있는 슬러그로 이름 짓는다, 예:
     `.omc/tasks/kisa-connector.md`, `.omc/tasks/epss-connector.md`). Write 도구로 그대로
     써서 셸 이스케이프를 아예 안 거치게 한 뒤, 각각
     `.\deploy\orchestrator\spawn-worker.ps1 -Task <슬러그> -PromptFile .omc\tasks\<슬러그>.md`
     로 스폰한다. 하위작업이 1개뿐이면 `-Prompt "..."` 인라인도 괜찮다.
     **작업량·독립성에 맞춰 워커 수를 늘린다** — 하나로 뭉쳐 맡기지 않는다.
     - 서로 다른 파일이면 무조건 나눈다. **같은 파일이라도** 건드리는 부분(줄 범위)이 겹치지 않으면
       나눈다 — 각 워커는 자기 워크트리(=자기 사본)에서 origin/main 을 기점으로 작업하므로 같은 파일의
       비겹침 구간을 여러 워커가 동시에 고쳐도 각자 PR 은 문제없다(병합 순서만 뒤에 신경 쓰면 됨).
     - "버그 4개 고쳐줘" 같은 요청은 **워커 1개에 4개를 욱여넣지 말고 워커 4개**로 쪼갠다. 워커 하나가
       크고 얽힌 작업을 하나 맡는 건 괜찮지만, 서로 무관한 하위작업을 한 워커·한 프롬프트에 모아
       넣지 않는다 — 프롬프트도 길어지고, 하나가 막히면 전체가 막힌다.
     기본 `-Permissions ask`(워커가 탭에서 사용자 승인). 믿고 맡길 작업만 `-Permissions skip`.
     프롬프트 문자열 자체는 (백틱·큰따옴표 등 특수문자 걱정 없이) 그대로 넘겨도 된다 —
     ...
     다만 그 방어는 spawn-worker.ps1 **내부**(파일→claude) 구간 얘기고, **메인이 PowerShell
     한 호출 안에서 긴 지시문을 인라인으로 조립하는 것 자체**는 별개 위험이라 위 파일 우선
     원칙으로 막는다 — 여러 줄 문자열을 커맨드라인에 직접 짜 넣다 오타·잘림이 난 전례가 있다.
...
- **PR 링크는 메인이 취합해서 보고한다.** 워커 탭이 자체적으로 "PR #N 생성됨" 을 보여줘도, 그건
  정보 제공용이지 병합 신호가 아니다 — 병합 여부는 메인이 모든 워커를 취합한 뒤 사용자에게
  확인받고 결정한다. 사용자가 워커 탭에서 곧장 병합하면, 메인이 뒤이어 같은 브랜치에 후속 커밋을
  얹다가 이미 닫힌 PR 과 연결이 끊기는 경합이 생긴다(실제로 겪음: PR #170/#171 분리 원인).
- **워커가 "완료"를 보고한 뒤 메인이 같은 브랜치에 후속 커밋을 얹기 전엔, 먼저
  `gh pr view <브랜치> --json state` 로 그 PR 이 이미 병합됐는지 확인한다.** 이미 병합됐으면
  같은 브랜치에 얹지 말고 별도 후속 PR 로 새로 낸다.
