...
- **스택은 서버 코드를 건드릴 때만 띄운다.** 워크트리를 만들었다고 반사적으로
  `dev up -d` 를 치지 않는다.
  - `agent/**`·`docs/**`·`*.md` 만 바꿨다 → **스택 불필요.** `bash -n` + 실제 컨테이너로 e2e 검증하면 된다
    (에이전트는 서버 코드가 아니라 스택을 띄워도 검사되는 게 없다). pre-push 도 자기 스택이
    없으면 스모크를 건너뛴다.
  - `server/**`·`db/**`·`tests/**` 를 바꿨다 → **전용 스택 필요.** `dev up -d` 후 스모크.
- **스택은 워크트리끼리 공유할 수 없다** — 재사용하려 들지 마라. `compose.dev.yml` 이
  `../server` 를 마운트하는데 이건 **compose 파일이 있는 그 트리**의 소스다. 컨테이너 하나는
  한 트리만 가리킨다. 남의 스택으로 스모크하면 **내 코드가 아니라 그 트리의 코드를 검사**하고
  초록불을 받는다 — 거짓이다. (pre-push 가 자기 스택만 치는 이유가 이것이다.)
- **스택을 띄웠으면 push 직후 내린다** — `./deploy/wt.sh rm <이름>`. "나중에 정리" 는 안 된다.
  스택 하나가 컨테이너 3개 + 포트 2개 + DB 볼륨이고, 워크트리가 쌓이면 그대로 곱해진다.
...
# 스택은 **서버 코드를 건드릴 때만** 띄운다(위 "스택은 서버 코드를 건드릴 때만" 참고).
#   agent/·docs/·*.md 만 고친다 → 이 두 줄을 건너뛴다.
./deploy/compose_runner.sh dev up -d   # 이 워크트리 전용 스택(공유 불가 — 트리별 소스를 마운트)
./tests/smoke.sh http://localhost:<할당된포트>

# 구현 → 검증 게이트 → 커밋 → push → PR
./deploy/wt.sh rm 무엇             # push 직후 정리(스택 down -v + 워크트리 제거)
```
