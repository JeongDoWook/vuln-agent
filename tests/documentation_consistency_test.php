...
if (count($documented) !== 53) {
    $fail('데이터베이스 요약은 운영 테이블 53개를 각각 한 번씩 열거해야 합니다: ' . count($documented));
}

preg_match_all('/^\s*entity (tb_[a-z_]+)/m', $erd, $erdMatches);
$entities = array_values(array_unique($erdMatches[1]));
if (count($entities) !== 52 || in_array('tb_schema_migrations', $entities, true)) {
    $fail('ERD는 마이그레이션 이력 테이블을 뺀 도메인 엔티티 52개여야 합니다');
}
