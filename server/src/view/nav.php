...
/**
 * 자산 상세(host.php) 상위 탭 ↔ 하위 탭 매핑의 SSOT — 정의는 여기 한 곳뿐이고 화면은 부르기만 한다.
 *   자산 상세 탭이 8~9개까지 늘어 한 줄에서 고를 수 있는 수를 넘겼다. 성격이 같은 것끼리 묶어
 *   상위 4개로 줄이되 **각 탭의 조회 로직은 그대로 둔다**(2단 탭이지 쿼리 통합이 아니다) —
 *   합치면 설치 패키지·컨테이너·런타임의 무거운 쿼리가 한 번에 돌고 페이지네이션도 엉킨다.
 *   탭 줄을 화면이 각자 조립하면 정의가 갈린다는 건 vg_findings_subtab_labels() 가 이미 겪었다.
 */
function vg_asset_tab_groups(): array {
    return [
        'risk'    => ['label' => '위험', 'tabs' => ['vuln', 'suppressed']],
        'config'  => ['label' => '구성', 'tabs' => ['packages', 'containers', 'runtime']],
        'norms'   => ['label' => '준거', 'tabs' => ['cce', 'accounts']],
        'history' => ['label' => '이력', 'tabs' => ['scans', 'manage']],
    ];
}

/**
 * 하위 탭 → 상위 탭 역산. URL 은 예전 그대로 하위 탭 키만 갖는다(?tab=cce) —
 *   저장소·북마크·외부 링크가 그 형태라 상위 탭 키를 URL 에 새로 얹지 않는다.
 */
function vg_asset_tab_group_of(string $tab): string {
    foreach (vg_asset_tab_groups() as $key => $group) {
        if (in_array($tab, $group['tabs'], true)) {
            return $key;
        }
    }
    return (string) array_key_first(vg_asset_tab_groups());
}

/**
 * 2단 탭 렌더. $tabDefs 는 지금 화면에 **존재하는** 하위 탭만 담는다
 *   (억제 0건이면 'suppressed' 가 아예 없다) — 없는 탭은 상·하위 어느 줄에도 안 나온다.
 */
function vg_asset_tabs(array $tabDefs, string $tab): void {
    $activeGroup = vg_asset_tab_group_of($tab);
    $top = [];
    $subs = [];
    foreach (vg_asset_tab_groups() as $key => $group) {
        $keys = array_values(array_filter(
            $group['tabs'],
            static fn(string $t): bool => isset($tabDefs[$t])
        ));
        if (!$keys) {
            continue;
        }
        /* 건수는 하위 탭이 하나뿐인 그룹에만 올린다. 여럿을 더하면 단위가 다른 값이 숫자 하나가
         *   된다 — '설치 패키지 811 + 컨테이너 22 + 런타임 146 = 979' 는 아무 뜻도 없다. */
        $top[$key] = [
            'label' => $group['label'],
            'n'     => count($keys) === 1 ? ($tabDefs[$keys[0]]['n'] ?? null) : null,
            // 상위 탭은 그 그룹의 첫 하위 탭으로 간다. 페이지·검색·계정필터는 탭마다 다른 목록을
            //   가리키므로 그룹을 옮길 때 버린다(남기면 빈 결과가 나온다).
            'href'  => vg_qs(['tab' => $keys[0], 'page' => null, 'epage' => null, 'q' => null, 'acc' => null]),
        ];
        if ($key === $activeGroup) {
            foreach ($keys as $t) {
                $subs[$t] = $tabDefs[$t];
    ...
    vg_subtabs($top, $activeGroup);
    // 고를 것이 하나뿐인 하위 줄은 그리지 않는다(억제 0건인 자산의 '위험' 그룹).
    if (count($subs) > 1) {
        echo '<div class="subtabs-sub">';
        vg_subtabs($subs, $tab);
        echo '</div>';
    }
}
