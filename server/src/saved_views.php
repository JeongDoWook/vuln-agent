...
function vg_saved_view_bar(PDO $pdo,string $page):void{
 ...
 $st=$pdo->prepare('SELECT id,name,query_json,is_default FROM tb_saved_views WHERE user_id=? AND page_code=? AND is_deleted=0 ORDER BY is_default DESC,name');$st->execute([$u['id'],$page]);$rows=$st->fetchAll();
