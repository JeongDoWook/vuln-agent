...
?>
  ...
  // 도넛에서 고른 등급(sev)은 검색 폼 필드가 아니라, 폼 제출 시 사라지지 않도록 hidden 으로 함께 싣는다.
  if ($sev !== '') {
  ...
  if ($scanId > 0 && !$scan) {
      ...
      $emptySpec = [
          'icon'  => 'block',
          'title' => '수집 #' . $scanId . ' 을(를) 찾을 수 없습니다.',
          'hint'  => '삭제됐거나 존재하지 않는 수집입니다.',
      ...
      //   이 화면(호스트별 최신 수집에서 매처가 실제로 잡은 판정)엔 없을 수 있다.
      $emptySpec = [
          'icon'  => 'search',
          ...
          'title' => "'" . mb_strimwidth($q, 0, 60, '…') . "' 는 이 화면(실제 수집·매칭된 현재 판정)에는 없습니다.",
          'hint'  => '벤더 판정·영향 패키지 목록은 실제 설치 여부와 무관한 전역 데이터라 다를 수 있습니다. '
                   . '등급·상태·조치 가능성 필터도 확인해 보세요.',
          'cta'   => $filterCta,
      ];
  } elseif ($hasAnyFilter) {
      // 검색어 없이 등급·상태·조치 가능성만으로 0건 — 도넛 조각 클릭으로 sev 가 걸린 경우
