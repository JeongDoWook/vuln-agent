...
if (!function_exists('vg_load_cve_catalog')) {
     ...
     *
     * tb_cve_affected_package 를 통째로 읽으면, RHEL OVAL 이 들어와 그 표가 50만 행이 된 뒤
     * 두 곳에서 한계에 닿는다 — 스캔마다 다시 읽어 30초 실행제한을 넘고, 전부 배열에 올려
     * 512MB 메모리를 초과한다. 스캔 하나가 보는 패키지는 수백 개뿐인데
     * 수십만 행을 들고 있을 이유가 없다.
     *
