 ...
 *   스케줄러 사이드카 컨테이너가 이전 실행 종료 후 60초 뒤 재호출한다(compose.yml — 고정
 *   주기가 아니라 고정 지연이라, 실행 자체가 오래 걸리면 실제 간격은 60초보다 길어진다).
...
//   없음" 조기 종료에 묶이면 OSV 가 0건이거나 미등록인 기간 내내 packages.php의 언어 패키지 탭
//   KPI 카드가 영원히 0으로 보인다(실측 확인됨). 스케줄러는 이전 실행 종료 후 60초 뒤 다시
//   도는 구간이라 여기서 매번 갱신해도 최신 데이터를 반영한다.
