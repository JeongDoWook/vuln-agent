...
<?php else: ?>
  ...
  <div class="card">
    ...
    if (count($r['paths']) === 1 && count($r['paths'][0]) === 1) {
        ...
        /* 하위 의존성/전체 트리 탭과 같은 가로 계층 트리 렌더러를 그대로 쓰되, 방향만
        ...
        vg_deptree_render($ancestorGraph, $target, $nodeBudget,
            ['path' => $ancPathMarks['nodes'], 'pathedge' => $ancPathMarks['edges']] + $treeCtx);
    }
    ?>
    </div>
  </div>
