...
$st = $pdo->prepare(
    "SELECT agent_command_id,status,progress_percent,progress_stage,progress_message,
            run_at,created_at,started_at,heartbeat_at,executed_at,cancel_requested_at,cancelled_at,
      ...
      ORDER BY FIELD(status,'running','pending','failed','done','cancelled'), created_at DESC LIMIT 1"
);
$st->execute([$hostId]);
$row = $st->fetch() ?: null;
