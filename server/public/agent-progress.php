...
if (!in_array($state, ['running', 'failed'], true)) { progress_fail(422, 'invalid state'); }
...
$sql = "UPDATE tb_agent_command
           SET status=?, progress_percent=?, progress_stage=?, progress_message=?,
               ...
               executed_at=IF(?='failed',NOW(),executed_at)
         WHERE agent_command_id=? AND host_id=? AND status IN ('pending','running') AND is_deleted=0";
$update = $pdo->prepare($sql);
$update->execute([$state, $percent, $stage, mb_substr($message, 0, 255), $state, $commandId, $hostId]);
