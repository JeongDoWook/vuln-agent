...
#### 6-1-a. 운영 확인 결과(2026-08-16) — **지우지 않는다. 이 항목은 닫는다.** ★

위 ①②를 운영에서 실행했다(**읽기 전용 `EXPLAIN`·`SELECT` 만**. DDL·설정 변경 없음).
결과는 **dev 결론을 뒤집었다.**

운영 `tb_finding` = **1,451,932행**(dev 118,544행의 12.2배). `idx_findings_is_deleted` 는
운영에도 그대로 있고 **Cardinality = 1** 로 dev 와 같은 퇴화 인덱스다. 그런데:

| | dev (118,544행) | **운영 (1,451,932행)** |
|---|---|---|
| 옵티마이저가 고른 인덱스 | `idx_findings_is_deleted` (rows 60,534) | **`idx_find_sev`** (rows 107,457) |
| 자율 실행 median | 205.98 ms | **111.1 ms** (114.7/110.2/111.1) |
| `FORCE INDEX (idx_find_scan_sev)` | 72.98 ms (**2.8배 빠름**) | **243.9 ms** (**2.2배 느림**) |

세 가지가 한꺼번에 뒤집혔다.

1. **운영은 문제의 퇴화 인덱스를 애초에 안 고른다.** ①의 판정 기준("key 가
   `idx_findings_is_deleted` 면 dev 와 같은 병")에 **해당하지 않는다.**
   지워도 이 질의의 계획은 안 바뀐다 — 이득이 0 이다.
2. **dev 의 해법이 운영에선 해악이다.** `idx_find_scan_sev` 를 강제하면 2.2배 느려진다.
   ②의 게이트("이득이 확인되면")를 통과하지 못한다.
3. **애초에 그만큼 안 느리다.** dev 236~255ms 로 잡혔던 이 집계가 운영에선 111ms 다.
   `compliance.php` 를 "2위 화면" 으로 만든 근거 자체가 운영엔 없다.

**그래서 `DROP INDEX idx_findings_is_deleted` 는 내지 않는다.** 145만 행 테이블의 인덱스를,
이득이 0 인데 되돌리기 비싼 방향으로 지울 이유가 없다.

이 결과가 §6-1 의 판단(“운영 확인 없이 최대 테이블의 인덱스를 지우지 않는다”)이 옳았음을
보여준다. **dev 수치만 보고 갔으면 이득 없는 DDL 을 운영 최대 테이블에 걸었을 것이다.**
#617 의 dev 0.65ms / 운영 1,554ms(2,392배)에 이어, **이 저장소에서 dev↔운영이 뒤집힌 두 번째
실측**이다. 다음에 성능 판단을 할 때 이 두 건을 근거로 삼는다.

*남는 질문(이번 범위 밖)*: 운영이 고르는 `idx_find_sev` 도 107,457행을 훑는다. 최적은 아닐 수
있으나 **111ms 는 이 화면에서 병목이 아니고**, 더 파려면 운영에서 계획을 흔드는 실험이
필요하다. 필요해지면 그때 별도로 잡는다.
