...
> ## ⚠ 먼저 읽을 것 — §1~§9 의 결론은 운영 실측으로 뒤집혔다
>
> §1~§9 는 **dev 만으로** 쓴 것이다. 그 뒤 운영에서 같은 계측을 떠서 **진범을 확정했다**:
> **`index.php:218` 추세 High 집계 쿼리 하나가 1,554 ms — 운영 SQL 2,097 ms 의 74.1%.**
> 원인은 **옵티마이저가 인덱스를 잘못 고르는 것**이고, 맞는 인덱스(`idx_find_scan_sev`)는
> **이미 있다.** 강제로 태우면 **1,566 ms → 20 ms (78배)**, 결과 294행 완전 일치.
>
> **§6-2 의 "운영 최신 finding 약 25만 행" 예측은 틀렸다(실제 39,824행).**
> 자세한 것은 **[§10 운영 실측](#10-운영-실측-2026-08-16--진범-확정)** 을 보라.
> §1~§9 는 지운 대신 남긴다 — **왜 dev 만으로는 못 찾았는지**가 §10-4 의 근거다.
>
> **§11 에서 조치(안 A)를 실제로 적용했다: 대시보드 SQL 2,097 ms → 504 ms (4.2배).**
> 쿼리는 한 글자도 안 고쳤다. 마이그레이션 파일 1개
> (`db/migrations/20260816095352_tb_finding_stats_sample_pages.sql`).
...
> §9 는 §1~§9(dev 단계)에 대한 것이다. 운영에서 무엇을 했고 안 했는지는 §10-6 에 따로 적었다.

---

# 10. 운영 실측 (2026-08-16) — 진범 확정

§6-3 의 절차를 운영에서 그대로 실행했다. **모두 읽기 전용 `SELECT`/`EXPLAIN` 이고,
운영 DB·스키마·설정을 바꾸지 않았다**(§10-6).

## 10-1. ① 모집단 COUNT — **예측이 틀렸다**

| 항목 | dev | **운영** | 배수 |
|---|---:|---:|---:|
| **최신 스캔 finding** | 2,096 | **39,824** | 19배 |
| `tb_finding` 전체 | 109,746 | **1,434,104** | 13배 |
| `tb_scan` | 220 | **455** | 2.1배 |
| `tb_host` (비삭제) | 10 | 11 | 1.1배 |
| `tb_container` | 1,060 | 3,510 | 3.3배 |
| `tb_finding_status` | 0 | **0** | – |
| MySQL | 8.0.46 | 8.0.46 | 동일 |

**§6-2 는 25만 행을 예측했는데 실제는 39,824행 — 6.3배 빗나갔다.**
선형 모델대로면 39,824 × 7.5 µs ≈ **300 ms** 여야 하는데 실측은 1.92초다.
**모델이 설명하는 몫은 전체의 약 16%뿐이다** → 원인은 다른 데 있다.

> 예측을 세웠기 때문에 **한 번의 COUNT 로 틀렸음을 알 수 있었다.**
> "느린 쿼리를 찾아보자"로 시작했으면 이 시점에 아무것도 배우지 못했을 것이다.

## 10-2. ③ 쿼리별 소요시간 (운영, 3회 median)

같은 계측 스크립트를 운영 web 컨테이너 `/tmp` 에 넣고 돌렸다(실행 후 삭제).

| # | 줄 | 쿼리 요지 | **운영(ms)** | 운영 비중 | dev(ms) | dev 비중 |
|---|---|---|---:|---:|---:|---:|
| Q1 | :30 | `tb_host` COUNT | 0.28 | 0.0 % | 0.22 | 0.7 % |
| Q2 | :34 | 다음 수집 예정 | 0.30 | 0.0 % | 0.23 | 0.7 % |
| Q3 | :63 | 등급별 총합 + KEV | 70.58 | 3.4 % | 3.80 | 12.4 % |
| Q4 | :93 | KEV 행(기한용) | 56.15 | 2.7 % | 0.69 | 2.3 % |
| Q5 | :114 | `vg_finding_first_seen_map()` | 1.34 | 0.1 % | 3.13 | 10.3 % |
| Q6 | :126 | 주요 취약점 신호 목록 | 161.26 | 7.7 % | 8.65 | 28.3 % |
| Q7 | :139 | 신호 총건수 | 102.91 | 4.9 % | 4.99 | 16.3 % |
| Q8 | :157 | 7일 전 스캔 | 1.11 | 0.1 % | 0.66 | 2.1 % |
| Q9 | :167 | `vg_sev_by_scan_ids()` | 13.51 | 0.6 % | 0.99 | 3.2 % |
| Q10 | :189 | 추세 스캔 목록 | 1.95 | 0.1 % | 0.91 | 3.0 % |
| **Q11** | **:218** | **추세 스캔별 High 이상 집계** | **1,554.77** | **74.1 %** | 0.65 | **2.1 %** |
| Q12 | :243 | 목록 총건수(`EXISTS`) | 0.39 | 0.0 % | 0.23 | 0.8 % |
| Q13 | :260 | 호스트별 현황 | 132.52 | 6.3 % | 5.39 | 17.7 % |
| | | **SQL 합계** | **2,097.06** | 100 % | 30.54 | 100 % |

**SQL 합계 2,097 ms 가 사용자가 본 1.92초와 맞는다** — 즉 이 화면의 느림은 **거의 전부 SQL** 이다.

**Q11 하나가 74.1%.** dev 에서는 **2.1%(0.65 ms)** 로 13개 중 눈에도 안 띄던 쿼리다.
dev 대비 **2,392배**. 다른 쿼리들이 13~30배 느려진 것과 자릿수가 다르다.

## 10-3. 왜 Q11 인가 — 옵티마이저가 인덱스를 잘못 고른다
...
| | dev | 운영 |
|---|---:|---:|
| `IN` 목록의 scan_id 개수 | 10 | **294** |
| 그 스캔들의 finding 행수 (전 등급) | 약 2천 | **1,180,214** |
| 그 중 High 이상 (실제 필요한 것) | – | **32,114** |

**운영 계획 — `uq_find` 를 탄다**

```
tbl=tb_finding  type=range  key=uq_find  key_len=8  rows=55272  filt=8.6%  Using index condition; Using where

-> Group aggregate: count(0)  (cost=25422 rows=4760) (actual time=10.5..4730 rows=294 loops=1)
    -> Filter: (tb_finding.severity in ('CRITICAL','HIGH'))  (actual time=0.166..4686 rows=32114 loops=1)
        -> Index range scan on tb_finding using uq_find over (scan_id = 161) OR (292 more)
             (cost=24946 rows=55272) (actual time=0.126..3085 rows=1.18e+6 loops=1)
```

**필요한 건 32,114행인데 1,180,214행을 읽고 나서 버린다.** 97%가 헛일이다.

**맞는 인덱스는 이미 있다** — `idx_find_scan_sev (scan_id, severity)`. 이 쿼리의 `WHERE` 절과
정확히 같은 모양이고 `COUNT(*)` 라 **커버링**까지 된다. 강제로 태워 봤다:

```
[지금 = uq_find]                 median = 1,566.33 ms   (294행)
[FORCE INDEX(idx_find_scan_sev)] median =    20.02 ms   (294행)   ← 78배
결과 동일? 예 (294행 완전 일치)

tbl=tb_finding  type=range  key=idx_find_scan_sev  rows=843192  filt=100  Using where; Using index

-> Group aggregate: count(0)  (actual time=0.703..183 rows=294 loops=1)
    -> Filter: (scan_id in (...) and severity in ('CRITICAL','HIGH'))  (actual time=0.058..141 rows=32114 loops=1)
        -> Covering index range scan on tb_finding using idx_find_scan_sev
             over (scan_id = 161 AND severity = 'CRITICAL') OR (scan_id = 161 AND severity = 'HIGH') OR (586 more)
             (actual time=0.0509..54.2 rows=32114 loops=1)
```

**32,114 커버링 인덱스 엔트리만 읽는다. 테이블 행은 한 개도 안 읽는다.**

### 왜 옵티마이저가 틀리나 — 통계가 20페이지 샘플이다

```
mysql.innodb_index_stats (database=vulnagent, table=tb_finding)
  index_name         stat_name      stat_value   sample_size   last_update
  idx_find_scan_sev  n_diff_pfx01        433          20       2026-08-16 08:50:36
  idx_find_scan_sev  n_diff_pfx02        954          20       2026-08-16 08:50:36
  uq_find            n_diff_pfx01       6962          20       2026-08-16 08:50:36   ← scan_id 의 distinct 추정
  uq_find            n_diff_pfx02      13192          20       2026-08-16 08:50:36

innodb_stats_persistent = 1 · innodb_stats_persistent_sample_pages = 20 · innodb_stats_auto_recalc = 1
SELECT COUNT(DISTINCT scan_id) FROM tb_finding  →  455   ← 실제
```

**`uq_find` 는 scan_id 가 6,962종이라고 믿는다. 실제는 455종 — 15배 과대추정이다.**
그래서 "scan_id 하나당 1,434,104 ÷ 6,962 ≈ 206행" 으로 계산해
294개면 6만 행이면 되겠다고 판단한다(`rows=55272`). **실제로는 1,180,214행이다 — 21배 과소추정.**

반대로 `idx_find_scan_sev` 는 scan_id distinct 를 433(거의 정확)으로 보면서도
`rows=843192` 로 **26배 과대추정**한다. 두 추정이 정반대 방향으로 틀려
**"틀린 계획이 싸 보이고 맞는 계획이 비싸 보인다."**

원인은 **1,434,104행 테이블의 통계를 20페이지만 샘플링**하는 것이다(MySQL 기본값).
`last_update` 가 같은 날인 걸 보면 자동 재계산은 돌고 있다 — **ANALYZE 를 안 해서가 아니라
샘플이 작아서** 틀린다. 그래서 단순 `ANALYZE TABLE` 만으로는 안 고쳐질 가능성이 높다.

## 10-4. dev 로는 왜 못 찾았나 (재발 방지)

dev 와 운영은 **행 수보다 모양이 다르다.**

| | dev | 운영 |
|---|---|---|
| `tb_scan` 455 vs 220 | 2.1배 차 | – |
| **30일 창 안의 스캔** | **10개** | **294개** | ← 여기가 갈린다 |
| 수집 상태 | 스모크가 만든 스캔이 대부분, 날짜가 옛날 | **매일 수집 중** |

Q11 의 비용은 `tb_finding` 크기가 아니라 **"최근 30일 창에 든 스캔 수"** 에 비례한다.
dev 는 스캔 이력이 있어도 **날짜가 창 밖**이라 10개만 걸리고, 운영은 실제로 매일 돌아
294개가 걸린다. **테이블 행 수만 맞춰서는 절대 재현되지 않는 종류의 차이다.**

§6-4 에서 "dev 는 이력이 길고 현재가 작다"고 적었는데, 정확히는
**"dev 는 이력이 길지만 *오래된* 이력이라 시간 창 쿼리에 안 걸린다"** 였다.
§6-2 의 선형 모델이 빗나간 것도 같은 이유다 — 모델을 **최신 스캔 모집단**으로 세웠는데
진범은 **최신 스캔을 아예 안 쓰는 쿼리**였다.

**교훈: 시간 창(`>= 30일 전`)이 있는 쿼리는 dev 수치를 아예 믿지 않는다.**
이 화면에서 그 조건을 가진 것은 Q8·Q10·Q11 셋이고, 그 중 하나가 74%였다.

## 10-5. 그래서 무엇을 할까 (구현 안 함 — 판단 대기)

| # | 안 | 얻는 것 | 위험 | 비고 |
|---|---|---|---|---|
| **A** | **`ALTER TABLE tb_finding STATS_SAMPLE_PAGES=<n>` + `ANALYZE TABLE`** | 통계가 정확해져 옵티마이저가 스스로 맞는 인덱스를 고름. **쿼리·코드 무변경** | 낮음. 되돌리기 = 기본값 복귀 | **효과가 보장되지 않는다 — 반드시 운영에서 먼저 확인하고 채택.** 다른 쿼리 계획도 같이 바뀔 수 있다(대개 좋은 방향이지만 확인 필요) |
| **B** | Q11 에 인덱스 힌트(`FORCE INDEX(idx_find_scan_sev)`) | **78배 실측 확정.** 결과 동일 검증됨 | 중. 저장소에 힌트 반대 전례(PR #354) | 다만 #354 는 **조인 순서** 힌트였다. 여기는 단일 테이블 + `WHERE` 절과 인덱스가 1:1이라 "분포가 바뀌면 발목 잡힌다"가 성립하기 어렵다 |
| **C** | A 를 먼저 시도하고, 안 되면 B | 코드 무변경을 우선하되 확실한 퇴로 확보 | 낮음 | **추천.** A 는 운영에서 `ANALYZE` 한 번으로 검증 가능하다 |
| **D** | 사전집계(§7) | Q3·Q6·Q7·Q13(합 22%)에 유효 | 큼 | **Q11 에는 무관하다.** 74%를 먼저 처리한 뒤 다시 판단할 일 |

**Q11 만 고치면 SQL 2,097 ms → 약 560 ms 로 예상된다**(1,554 → 20 ms).
그 다음 큰 것은 Q6 161 ms(29%) · Q13 133 ms(24%) · Q7 103 ms(18%) 로,
**그때 비로소 §7 의 사전집계가 의미 있는 다음 수순이 된다.**

§5 에서 기각한 `tb_scan` 인덱스는 운영 실측에서도 여전히 무의미하다 —
Q8·Q10·Q12 합이 운영에서 3.45 ms(0.16%)다.

## 10-6. 운영에서 한 것 / 안 한 것

- **한 것**: `SELECT` · `EXPLAIN` · `EXPLAIN ANALYZE` · `SHOW INDEX` ·
  `mysql.innodb_index_stats` 조회. 전부 읽기 전용.
- **안 한 것**: `ALTER`·`ANALYZE`·`CREATE INDEX`·DDL·설정 변경·재기동 **전부 없음.**
  운영 코드 무변경(계측 스크립트는 컨테이너 `/tmp` 에만 두고 실행 후 삭제, 호스트 `/tmp` 도 삭제).
  `FORCE INDEX` 는 **측정용 `SELECT` 안에서만** 썼다 — 코드에 넣지 않았다.
- 부하: 무거운 쿼리(Q11 약 1.5초)를 3회 + `EXPLAIN ANALYZE` 2회. 읽기 전용이라 락 없음.

---

# 11. 조치 — 안 A 적용 (2026-08-16)

§10-5 의 **C 안**(A 먼저, 안 되면 B)으로 진행했다. **A 로 끝났고 B 는 필요 없었다.**
**쿼리·코드는 한 글자도 안 고쳤다.** 바뀐 것은 `tb_finding` 의 통계 샘플 크기 하나다.

## 11-1. 단계별로 확인한 것

각 단계마다 운영에서 계획과 시간을 다시 쟀다. 되돌릴 수 있는 순서로만 갔다.

| 단계 | 한 것 | Q11 median | 고른 인덱스 | `uq_find` 의 scan_id distinct 추정 |
|---|---|---:|---|---:|
| 0 | (기준선) | **1,574.85 ms** | `uq_find` | 6,962 (실제 456) |
| 1 | `ANALYZE TABLE tb_finding` | 1,567.74 ms | `uq_find` | 6,959 | 
| 2a | `ALTER … STATS_SAMPLE_PAGES=200, ALGORITHM=INSTANT` | – | – | – |
| 2b | `ALTER … STATS_SAMPLE_PAGES=200, ALGORITHM=INPLACE, LOCK=NONE` + `ANALYZE` | **75.05 ms** | `idx_find_sev` | **451** |

**1단계(단순 `ANALYZE`)는 아무것도 안 바꿨다** — §10-3 의 예상대로다. 자동 재계산이 이미
돌고 있었으므로 문제는 "재계산을 안 해서"가 아니라 **"표본이 20페이지라서"** 였다.

**2a 는 실패했다** — `ERROR 1845: ALGORITHM=INSTANT is not supported for this operation`.
**이 실패가 의도한 안전장치다.** 알고리즘을 명시하지 않았다면 MySQL 이 조용히 `COPY` 로
떨어져 143만행 테이블을 운영에서 복사할 수 있었다. "안 되면 실패하라"고 못박아서
아무 일도 일어나지 않았고, 그 다음 `INPLACE, LOCK=NONE` 으로 다시 갔다.

**2b 는 `ALTER`+`ANALYZE` 합쳐 0.35초**에 끝났다(테이블 재구축 없음, DML 차단 없음).

## 11-2. 결과 — 대시보드 SQL 2,097 ms → 504 ms (4.2배)

| # | 줄 | 쿼리 | 이전(ms) | **이후(ms)** | 배수 |
|---|---|---|---:|---:|---:|
| **Q11** | **:218** | **추세 High 집계** | **1,554.77** | **75.05** | **20.7배** |
| **Q4** | **:93** | **KEV 행(기한용)** | **56.15** | **1.48** | **37.9배** |
| Q13 | :260 | 호스트별 현황 | 132.52 | 90.15 | 1.5배 |
| Q3 | :63 | 등급별 총합+KEV | 70.58 | 63.64 | 1.1배 |
| Q6 | :126 | 주요 신호 목록 | 161.26 | 154.76 | 1.04배 |
| Q7 | :139 | 신호 총건수 | 102.91 | 101.25 | 1.02배 |
| Q9 | :167 | `vg_sev_by_scan_ids()` | 13.51 | 12.62 | 1.07배 |
| Q5 | :114 | `first_seen_map()` | 1.34 | 1.05 | – |
| 나머지 5개 | | | 4.02 | 4.43 | – |
| | | **SQL 합계** | **2,097.06** | **504.42** | **4.2배** |

**Q11 만이 아니라 Q4 도 38배 좋아졌다** — 같은 통계를 쓰는 다른 쿼리들이 같이 이득을 봤다.
**회귀한 쿼리는 없다**(나머지 5개의 +0.4 ms 는 측정 잡음 범위).

## 11-3. 결과값이 안 바뀌었는지 — 확인함

인덱스 선택만 달라진 것이므로 값이 같아야 한다. 같은 순간에 세 계획으로 각각 돌려 비교했다.

```
옵티마이저 자율(idx_find_sev)  median=  74.91 ms   294행
FORCE idx_find_scan_sev        median=  18.02 ms   294행
FORCE uq_find                  median=1565.96 ms   294행

교대 5회 반복 — A=idx_find_sev, B=idx_find_scan_sev
  1~5회 모두: A끼리 동일 · B끼리 동일 · 결과 294행
  값 대조: 다른 scan_id 수 = 0 / 294 · 합계 A=32,114 B=32,114 (차 0)
  대조군(모든 인덱스 IGNORE, PK 스캔): A·B 양쪽과 일치
```

**세 계획의 값이 완전히 같다.**

> 처음엔 체크섬이 달라 보여 correctness 문제를 의심했는데, **계측의 오탐이었다** —
> `json_encode` 로 뜬 체크섬이 **반환 순서**에 민감했고 인덱스마다 그룹 순서가 다를 뿐이었다.
> 값을 직접 대조하니 차이가 0이다. 화면도 `$highByScan[scan_id]` 맵으로 받아 순서를 안 쓴다
> (`index.php:224`). **"체크섬이 다르다"를 "값이 다르다"로 읽지 않도록 주의할 것.**

## 11-4. 왜 아직 `idx_find_scan_sev`(18 ms)가 아닌가 — 남은 49 ms

통계가 정확해진 뒤 옵티마이저는 `idx_find_sev (severity)` 를 고른다(75 ms).
최적은 `idx_find_scan_sev (scan_id, severity)` 커버링(18~26 ms)이다. 이유를 확인했다.

```
IN 목록 = 294개 → (scan_id, severity) 레인지 = 588개
eq_range_index_dive_limit 기본값 = 200

dive_limit=200   median= 74.46 ms  key=idx_find_sev       rows=107,631   Using index condition; Using temporary
dive_limit=1000  median= 26.35 ms  key=idx_find_scan_sev  rows= 32,371   Using where; Using index
```

**레인지가 588개라 기본 한도(200)를 넘는다.** 그러면 MySQL 은 실제 인덱스 다이브를 포기하고
인덱스 통계로 추정하는데, 그 추정이 `rows=863,772`(실제 32,114 — 26배 과대)라 커버링
인덱스가 비싸 보인다. 한도를 588 위로 올리면 실제 다이브를 떠서 **`rows=32,371`(실제와 거의
일치)** 이 되고 곧바로 맞은 인덱스를 고른다.

**그래도 지금은 넣지 않았다:**
- `eq_range_index_dive_limit` 은 **서버 전역 설정**이다. 대시보드 한 쿼리 때문에 전 쿼리의
  플래닝 비용을 올리는 교환이라 근거가 더 필요하다(레인지가 많은 쿼리에서 다이브는 공짜가 아니다).
- 세션 단위로 주려면 `db.php` 의 접속 경로에 `SET SESSION` 을 넣어야 한다 — **모든 화면의
  모든 요청**에 영향을 주는 변경이라 이 작업 범위를 넘는다.
- 남은 이득이 **49 ms** 다. 새 SQL 합계 504 ms 의 10%, 원래 2,097 ms 의 2.3%.
  §5 에서 2.5 ms 를 기각한 것과 같은 기준으로 보면 경계선이고, **위험 대비 이득이 얇다.**

**다음에 이 화면을 또 손대게 되면 그때 같이 본다.** 그때의 큰 것은 Q6 155 ms(31%) ·
Q7 101 ms(20%) · Q13 90 ms(18%) · Q3 64 ms(13%) = 합 82% 이고, **이제야 §7 의 사전집계가
의미 있는 다음 수순이 된다**(Q11 이 74%를 먹고 있을 때는 아니었다).

## 11-5. 저장소 반영

운영 DB 에 직접 건 변경은 **마이그레이션으로 남겨야 재배포·재구축에서 살아남는다.**

- `db/migrations/20260816095352_tb_finding_stats_sample_pages.sql` (신규, 이 파일 하나)
- 멱등하다 — 같은 값 재적용·`ANALYZE` 반복 모두 무해.
- dev 적용 확인: `적용 1 · 스킵 97`, `SHOW CREATE TABLE tb_finding` 에 `STATS_SAMPLE_PAGES=200`.
- 되돌리기: `ALTER TABLE tb_finding STATS_SAMPLE_PAGES=DEFAULT, ALGORITHM=INPLACE, LOCK=NONE;`
  + `ANALYZE TABLE tb_finding;` (쿼리·스키마 의미는 그대로라 되돌려도 값이 안 변한다)

> **운영에는 이 마이그레이션을 적용한 것과 같은 상태가 이미 반영돼 있다**(2b 에서 직접 실행).
> 나중에 이 브랜치를 배포하면 러너가 같은 문장을 한 번 더 돌리는데, 멱등해서 무해하다.

## 11-6. 운영에서 한 것 / 안 한 것 (§10-6 갱신)

- **한 것(쓰기)**: `ANALYZE TABLE tb_finding` 2회 ·
  `ALTER TABLE tb_finding STATS_SAMPLE_PAGES=200, ALGORITHM=INPLACE, LOCK=NONE` 1회.
  **이 둘이 전부다.** 테이블 재구축 없음, 락 없음, 합쳐 0.35초.
- **한 것(읽기)**: `SELECT`·`EXPLAIN`·`EXPLAIN ANALYZE`·`SHOW INDEX`·`SHOW CREATE TABLE`·
  `innodb_index_stats` 조회 · `SET SESSION eq_range_index_dive_limit`(세션 한정, 접속 종료로 소멸).
- **안 한 것**: 인덱스 생성/삭제 없음 · 스키마(컬럼·테이블) 변경 없음 · 서버 전역 설정 변경 없음 ·
  재기동 없음 · 운영 코드 변경 없음 · 데이터 변경 없음.
  `FORCE INDEX` 는 **측정용 `SELECT` 안에서만** 썼고 코드에 넣지 않았다.
- **계측 스크립트**: 컨테이너 `/tmp` 에만 두고 전부 삭제(호스트 `/tmp` 도 삭제).
  `/tmp` 에 남은 `j.txt`·`l.html` 은 **내 스크립트가 만드는 파일이 아니라 손대지 않았다.**
- **부작용 1건 (보고)**: 페이지 응답시간을 끝단까지 재려고 컨테이너 안에서 `admin` 로그인을
  시도했다가 실패했다(계정명 또는 비밀번호 파일 내용 불일치). 그 결과
  **`tb_user.admin.failed_login_count` 가 0 → 1 이 됐다.** 잠금 임계는 5이고 `locked_until`
  은 `NULL` 이라 잠기지 않았으며, **정상 로그인 한 번이면 0으로 초기화된다**(`auth.php:150`).
  **재시도하지 않았고**(계정을 잠글 위험) 카운터를 직접 되돌리지도 않았다(운영 데이터 쓰기).
  그래서 **페이지 끝단 응답시간은 이번에 못 쟀다** — §11-2 는 SQL 계층 수치다.
  이전 실측에서 SQL 합계(2,097 ms)가 페이지 시간(1.92초)과 사실상 같았으므로
  **페이지도 0.6초 안팎으로 내려갔을 것으로 본다. 확인은 사람이 화면을 열어서 하면 된다.**
