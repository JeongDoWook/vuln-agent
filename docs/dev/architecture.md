...
- **웹은 스캔을 직접 돌리지 않는다.** 화면은 `pending` 행만 만들고 집행은 **스케줄러 틱**(1분)이나
  ...
  (기본 1건·45초) 수집 틱을 굶기지 않는다. 예전엔 집행하는 곳이 아예 없어 "지금 스캔"이 영원히
  대기했다.
...
(**"직접 조치 불가 — X 가 끌어옴"으로 대체** + 경로 링크. 그 패키지만 갈아끼우면 부모가 깨진다) /
...
않는다**: 설치되지 않은 부모 버전이 무엇을 끌어오는지는 수집물로 알 수 없다(purl 과 같은 원칙 —
...
준수로 표기하면 심사 증빙에 **허위 안심**을 싣는다. 그래서 보유 스캔 이력이 SLA 보다 짧아 위반을
...
**범위**: 도메인 엔티티 **59개 전부**(= 전체 60테이블 − `tb_schema_migrations`, 마이그레이션 러너
