...
**잘린 사실을 화면에 밝힌다**. **노드-링크 그래프 라이브러리**(d3-force·cytoscape 류)는 들이지
않는다 — 이 의존성 관계는 접이식 목록으로 충분하다(KISS). 이건 이 화면의 결정이지 차트 전반의
금지가 아니다: 대시보드 차트는 `assets/vendor/chartjs`(로컬 서빙, `vg_chart()`)를 쓴다.
...
**정책·승인이력처럼 vuln-agent 가 갖고 있지 않은 근거가 필요한 통제는 판정하지 않는다** —
남은 4개(정책문서·접근권한 검토·사고대응·재해복구, `VG_COMPLIANCE_MANUAL_CHECKLIST`)는 증적이 제품
밖에 있어 사람이 직접 심사해야 한다. 2026-08-18 부터는 화면에도 세우지 않는다(체크리스트 카드 제거) —
확인해 줄 수 있는 게 없는 항목을 자동판정 통제와 나란히 두면 화면이 "체크되지 않은 것" 목록이 된다.
상수·조항 매핑은 그대로 남겨 두고 심사 항목은 `docs/dev/화면-안내.md` 가 갖는다(삭제가 아니라 강등).
...
**데이터**(수집 상태/CVE/패키지/판정 근거/CCE), 관리(사용자/권한/에이전트 키/
...
**'데이터' 의 다섯 번째 자리는 SSG 룰 카탈로그에서 CCE(`cce-rules.php`)로 바뀌었다.**
...
- **설정형 RBAC**: `admin` 은 코드에서 항상 전체 허용(잠금 방지)이라 권한 행을 두지 않는다.
  ...
  **메뉴코드는 사이드바 링크와 1:1 이다.** 예전엔 `findings` 하나가 링크 6개(탐지 결과·컴플라이언스·
  ...
  함께 열리므로(CVE 상세 ← 탐지 결과·CVE·보안 공지, 자산 상세 ← 자산·탐지 결과)
