...
> CCE 39항목 · 기반시설 U-코드 72항목 · 도메인 테이블 62개.
...
| 항목 | 내용 |
|---|---|
| 상시 데몬(systemd 있음) | `vuln-agent.service` — `run.sh` 가 poll 처리(수집이 걸리면 그 수집까지) 종료 후 10초 뒤 `agent-poll.php` 를 재poll 하고 초기 주기(기본 60분, `--schedule`)로 정기수집 시작 |
...
**범위**: 도메인 엔티티 **61개 전부**(= 전체 62테이블 − `tb_schema_migrations`, 마이그레이션 러너
