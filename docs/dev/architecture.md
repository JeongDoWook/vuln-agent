...
> 현행 기준: 2026-08-11 · 에이전트 3.13 · pull 명령 큐, 진행 heartbeat/취소, 관리 IP 보고,
...
**통제 기준 매핑(`control_mapping.php`)은 그 서브탭에서 내려왔다 — 강등이지 삭제가 아니다.**
`compliance.php` 의 `vg_subtabs()` 호출을 없애고 본문 **맨 아래**의 링크 한 줄만 남겼으며,
`control_mapping.php` 도 자기 탭바를 지우고 `← 컴플라이언스 매핑` 링크로 바꿨다. 사이드바 항목의
`active_keys` 에서도 `control_mapping` 이 빠졌다. 판정을 하지 않는 화면(같은 CCE 결과를 기준별로
묶어 세기만 한다)이라 상시로 볼 필요가 없어 **발견성만 낮춘 것**이고, 파일·인가 게이트·URL·
302 리다이렉트는 전부 그대로 살아 있다(링크까지 지워 고아 페이지로 만들지는 않았다). 탭 줄의 라벨·순서·목적지는 `vg_findings_subtab_labels()`/`vg_findings_subtabs()`
