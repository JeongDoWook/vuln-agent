...
web·scheduler 의 환경변수는 `deploy/compose.yml` 맨 위 **`x-app-env` 앵커(`&app-env`)** 한 곳에 모이고
두 서비스가 `environment: *app-env` 로 참조한다. 값은 **서비스의 `environment:` 가 아니라 앵커에
넣는다** — 서비스 쪽에 직접 쓰면 두 곳으로 갈라져 어긋난다.

```yaml
x-app-env: &app-env                              # deploy/compose.yml
...
> **제거된 설정**: `UI_DETAIL_PER_PAGE_DEFAULT` — 자산 상세 목록도 `UI_PER_PAGE_DEFAULT` 를 쓴다(PR #681).
...
**세션 만료와 토큰 유효기간은 환경변수가 아니다** — 세션 만료는 DB 설정([운영 설정](#운영-설정-db--tb_setting)),
나머지는 코드 상수다.
...
에이전트가 보낸 `X-Agent-Timestamp` 가 서버 시각과 이 값 이상 벌어지면 요청을 거부한다. NTP 가 안
맞는 망에서 수집이 거부되면 이 값을 올린다(그만큼 재전송 방어 창도 넓어진다).
