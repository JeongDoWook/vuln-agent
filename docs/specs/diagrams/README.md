...
>
> 아래 6장의 도메인 분리본(`erd-*.puml`/`.svg`)도 같은 생성기(`split_erd_domains()`)가 `erd.puml`
> 갱신 직후 같은 실행 안에서 함께 만드는 산출물이다 — **손으로 고치지 않는다**는 규칙이 그대로 적용된다.
...
전체 61테이블을 한 그림에 담으면 GitHub 본문 폭(약 900px)에서 실효 글자 크기가 4.6px 로 떨어져
...
전체 화면을 한 그림에 담으면 GitHub 본문 폭(약 900px)에서 실효 글자 크기가 7.5px 로 한계 이하라,
아래 5장의 영역별 그림으로 나눠 싣는다. 옅게 칠한 화면은 그 묶음이 아니라 다른 묶음이 다루는
...
에이전트가 poll 완료 후 10초 뒤 다시 부르는 poll 에 자기 버전을 실어 보내는 데서 시작해, 노드가 무엇을 어떤 순서로
