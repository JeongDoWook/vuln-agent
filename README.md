...
- **계정 인벤토리** — 실제 계정 목록(UID·셸·잠금·최근 로그인·sudo 권한)을 ISMS-P 2.5.x·N2SF 계정관리 관점으로
  판정합니다. 공유계정·미사용 계정은 단정하지 않고 확인 항목으로 표시하며, 패스워드 해시는 수집하지 않습니다.
...
- **보안 설정 점검과 무결성 검증** — CCE/SSG 기반 설정(SSH·계정·권한·시간동기화·로그·암호화)을 PASS·FAIL·
  판정 불가로 구분하고 ISMS-P·기반시설 U-코드·N2SF 기준으로 매핑합니다. `rpm -Va`/`dpkg --verify` 무결성
  검증은 비용이 커서 선택 기능이며, 켜지 않은 자산은 "정상"이 아니라 **미수행**으로 구분합니다.
- **컴플라이언스 매핑** — 패치관리·정보자산 식별·보안시스템 운영·계정 관리 4개 통제를
  KISA ISMS-P·ISO 27001 기준으로 자동 판정하고, 증적이 제품 밖에 있는 항목은 체크리스트로 둡니다.
  판정 근거가 부족하면 준수가 아니라 **판정 불가**로 두고, SLA 기준일은 설정 화면에서 조직에 맞게 바꿉니다.
  하루 1건 스냅샷을 남겨 심사 시점의 준수 상태를 날짜별로 되짚을 수 있습니다.
- **오픈소스 구성요소 분석(SCA)와 SBOM 산출** — pip/npm/gem/composer/maven/nuget/cargo/go 8개 언어
  생태계 패키지(Go는 바이너리 buildinfo까지)를 CVE와 매칭하고 라이선스를 허용적·카피레프트·미상으로
  분류합니다. 자산별 부품표는 CycloneDX 1.5·SPDX 2.3으로 내보냅니다.
- **함대 운영** — 웹에서 자산별 즉시·예약 수집, 주기 변경, 진행률 확인과 실행 중단이 가능합니다.
- **변화와 감사 이력** — 실제 수집 실행, 패키지 변화, 취약점 변화와 사용자 조작을 추적합니다. 접속기록은
  접속일시·식별자·접속지 IP·처리 대상·수행업무 5요소로 남기며 기간·행위·대상별로 조회할 수 있습니다.
...
- 전체 자산 위험 현황과 우선순위
- 호스트별 취약점, 설치 패키지(무결성 검증 결과 포함), 프로세스, 열린 포트, 계정과 컨테이너 대장
- 취약점별 CVSS·EPSS·KEV, 영향 위치, 수정 버전과 판정 근거
- 백포트로 억제된 항목, 재시작·재부팅 필요 항목, 조치 상태와 미조치 사유·승인자
- 벤더가 고치지 않는 CVE가 몰려 제거·대체를 검토할 패키지
- 전체 자산의 설치 패키지 통합 검색, 언어 패키지 라이선스 위험도, 의존성 그래프(무엇이 이 패키지를 끌어왔나)와 전이 의존성이라 직접 고칠 수 없는 취약점 — 부모 하나를 올리면 함께 해결되는 건수까지
- 자산별 중요도와 N2SF 보안등급(확정값·시스템 제안값)
- 컴플라이언스 통제별 준수 판정과 날짜별 추이, 통제 기준(ISMS-P·U-코드·N2SF)별 점검 매핑
- 외부 보안 피드 수집 상태와 실행 이력
- 역할 기반 권한, 호스트별 에이전트 키(유효기간 포함), 5요소 접속기록과 감사 이력
...
시스템 제안은 수집한 **시스템 사실**을 등급 검토 신호로 해석한 초안일 뿐, 확정 판정이 아닙니다.
...
| **S · 민감** | `fluentd`·`filebeat`·`restic`·`bacula` 등 지정된 로그수집·백업 프로세스가 실행 중 | 전달 전용인지 저장소인지, 일회성 작업인지, 수집·백업 대상의 내용 |
...
근거하며, 데몬 탐지만으로 법적 등급이 확정되지는 않습니다. **외부 노출도 공개 가능성의 신호일 뿐
공개해도 된다는 판정이 아닙니다.** S와 O가 함께 나오면 S를 제안하고, 어느 규칙에도 안 걸리면
**아무것도 제안하지 않습니다**(`–`는 "등급 없음"이 아니라 "확정도 제안도 없음").
