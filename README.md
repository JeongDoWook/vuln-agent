...
| 기능 | 설명 | 더 보기 |
|---|---|---|
| 판정 | 런타임 노출을 `EXTERNAL`(외부 노출)부터 `INSTALLED`(설치만 됨)까지 7단계로 구분해 같은 CVE라도 위험도를 다르게 매기고, 오탐 억제는 사유·근거를 함께 남기며 KEV·EPSS로 우선순위를 정함 | [아키텍처 §2](docs/dev/architecture.md) |
...
| 설정 점검·컴플라이언스 | CCE를 PASS·FAIL·판정 불가로 구분, ISMS-P·ISO 27001 자동 판정(3개 통제 자동화·나머지는 체크리스트), 기반시설 U-코드 72개를 분모로 둔 커버리지 | [보안설정 조치 가이드](docs/dev/보안설정-조치가이드.md) |
...
| AI 위험 분석 보고서(선택) | 결정론적 위험점수·우선순위 위에 로컬 LLM이 한국어 서술을 얹은 PDF 보고서 생성, 렌더링 직전 자체 검증 게이트로 환각·번역누락 차단 — 기본 꺼짐, 별도 인프라 필요 | [아래 §AI 위험 분석 보고서](#ai-위험-분석-보고서-선택-기능-기본-꺼짐) |
