...
- 전체 자산 위험 현황과 우선순위
- 호스트별 취약점, 설치 패키지, 프로세스, 열린 포트, 컨테이너와 계정 인벤토리
...
시스템 제안은 수집 데이터에서 확실한 근거를 찾은 경우에만 붙습니다(우선순위 순):

| 근거 | 제안 |
|---|---|
| 로그 수신 데몬(rsyslog·syslog-ng 등)이 루프백 외 포트를 열고 있음 | **S** |
| 로그수집기·백업 데몬(fluentd·filebeat·restic·bacula 등) 실행 중 | **S** |
| 외부(EXTERNAL)에서 닿는 노출 포트가 있음 | **O** |

앞의 두 규칙은 「정보공개법」 제9조의 "로그 및 임시백업 등"이 명시적 S인 데서 옵니다.
외부에 열려 있다는 사실이 "공개해도 되는 정보"를 뜻하지는 않으므로 S가 O보다 우선합니다.
셋 중 어디에도 걸리지 않으면 **아무것도 제안하지 않습니다** — 목록의 `–` 는 "등급 없음"이 아니라
"확정도 제안도 없음"입니다. C 자동 제안 규칙은 없습니다(업무정보의 내용은 수집물로 알 수 없습니다).
