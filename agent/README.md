...
> ## ⚠ 3.14 로 올라갈 때는 **노드에서 한 번 갱신 작업이 필요하다**
>
> 3.14 는 중앙 응답 파싱·수집 인자 조립을 `run.sh` 에서 `vuln-inventory-agent.sh --poll-once`
> 로 옮겼다. **`run.sh` 는 자동 업데이트 대상이 아니다**(자동 갱신은 본체 하나만 교체한다) —
> 그래서 본체가 3.14 로 자동 갱신돼도, 옛 `run.sh` 는 `--poll-once` 를 부르지 않고 예전
...
> 3.14 부터는 중앙이 이 상태를 **`미지원` 으로 화면에 표시**하므로, 어떤 노드가 안 갱신됐는지는
...
| 버전 | 들어간 것 |
|---|---|
| 3.14 | **명령 처리 로직을 `run.sh` → 본체(`--poll-once`)로 이전** — 응답 파싱·수집 인자 조립이 자동 업데이트를 타게 됐다. `install-agent.sh --runner-only`, `agent_push.sh --with-runner` 추가 |
...
**애초에 `run.sh` 에 로직을 두지 않는다.** 3.14 부터 응답 파싱·수집 인자 조립은
...
- 자동 업데이트가 바꾸는 것은 **본체 하나뿐이다.** `run.sh` 는 바뀌지 않는다 — 그래서 3.14 는
  로직을 본체로 옮겼다(위 ⚠ 참고).
...
| `미지원` 표기 | 중앙이 무결성 포함으로 명령을 걸었는데 **그 명령으로 생성된 스캔에 무결성 결과가 없을 때** 뜬다 — 노드의 `run.sh` 가 옛 버전이라 `--verify-files` 를 안 붙였다는 뜻이다(위 ⚠ 3.14 참고). `rpm`·`dpkg` 가 둘 다 없는 노드도 같은 상태가 된다 |
