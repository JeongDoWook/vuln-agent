...
3. `bash -n` 문법 검사 통과 시 기존 스크립트를 `.bak` 으로 백업하고 `mv` 로 원자적 교체.
4. 다음 실행(정기수집 또는 예약 명령)이 시작되기 **전에** 새 스크립트 자기점검(`bash -n` +
...
5. 적용 결과(성공/실패/롤백/건너뜀)는 다음 poll GET 에 실어 서버로 보고되고, 서버는 감사로그
...
### 보안 한계 — sha256 은 "전송 오류 방지"이지 "서버 침해 방어"가 아니다

지금의 sha256 검증은 **다운로드가 중간에 깨지지 않았는지**만 보장한다. 배포 서명(웹앱 프로세스와
분리된 키로 서명하고 에이전트가 그 서명을 검증하는 것)은 **아직 없다** — sha256 값 자체를 같은
`agent-poll.php`(웹앱)가 계산해서 내려주므로, 웹 티어가 침해되면 공격자는 자신이 준비한 악성
스크립트의 해시를 그대로 응답에 실어 "검증 통과"로 만들 수 있다. 즉 지금 구조에서 **웹 티어 침해
1건은 이 스크립트가 배포된 전체 노드의 root 침해로 이어질 수 있다.** 이번 라운드에서는 최소한의
차단선(HTTPS 강제 — 위 1번)만 반영했고, 진짜 서명 체계는 별도 라운드로 남겨 두었다. 그 전까지는
`SEND_URL` 이 반드시 HTTPS 여야 자동 업데이트가 동작한다는 점, 그리고 자동 업데이트가 이 한계를
안고 있다는 점을 알고 운영한다.
