...
| `openssl` | **아니오**(수집·전송엔 안 쓴다) | 자동 업데이트가 **적용되지 않는다** — 서명을 검증할 도구가 없어 `no_verify_tool` 로 보고하고 구버전에 그대로 머문다(조건별 동작은 아래 **"자동 업데이트의 예외 상황"** 절 한 곳에만 적는다). 수집·전송은 그대로 |
...
`run.sh` 가 남기는 **보고값은 전부 여기 있다** — 로그·감사로그(`tb_activity_log`,
`agent_auto_update`)에서 본 값을 이 절에서 찾으면 된다. 성공(`ok`)·롤백(`rollback`)·
재시도 안 함(`skipped_known_bad`)은 교체 이후 자기점검 경로라 위 6번 항목에 있고,
그 앞에서 **멈추거나 저하되는** 경우가 아래 세 표다.

#### 호스트 환경 때문에 멈추거나 저하되는 경우

| 상황 | 적용되나 | 동작·보고값 | 왜 그렇게 했나 |
|---|---|---|---|
| **`sha256sum`·`shasum` 이 둘 다 없는 노드** | **아니오** | 다운로드까지는 하지만 해시를 계산할 수단이 없어 `no_sha_tool` 로 보고하고 **적용하지 않는다.** 다음 poll 에 다시 시도한다 | 무결성 검증을 못 하는 상태에서 적용하면 깨진(또는 바꿔치기된) 스크립트를 그대로 실행하게 된다. 검증 불가는 검증 면제가 아니다 |
| **`openssl` 이 아예 없는 노드**(busybox·minimal 컨테이너 등) | **아니오** | `have openssl` 에서 걸려 **적용하지 않고** `no_verify_tool` 로 보고한다. 구버전 그대로 돌면서 다음 poll 에 다시 시도한다 | 서명 검증은 sha256 통과와 **별개로** 반드시 통과해야 한다 — 검증할 수단이 없는 것은 "검증 면제"가 아니라 "검증 불가"다. 조용한 스킵이 아니라 로그·보고로 남긴다(바로 위 `no_sha_tool` 행과 같은 클래스다 — "필수 도구 없음 → 적용 안 됨"). 자동 갱신을 받으려면 그 노드에 `openssl` 을 넣어야 한다 |
...
#### 내려받은 것이 기대와 달라 멈추는 경우 — **보안 관점에서 먼저 볼 값들이다**

| 상황 | 적용되나 | 동작·보고값 | 왜 그렇게 했나 |
|---|---|---|---|
| **체크섬 불일치** — 받은 파일의 sha256 이 poll 응답의 기대값과 다름 | **아니오** | `checksum_mismatch` 로 보고하고 받은 파일을 지운다. 다음 poll 에 다시 시도한다 | 전송 중 손상이면 재시도로 풀린다. **재시도해도 계속 나면 손상이 아니라 중간자·배포본 교체를 의심**한다 — 기대값을 내려준 쪽과 파일을 내려준 쪽이 같은 서버라 어긋날 이유가 없다 |
| **서명 검증 실패** — Ed25519 서명이 고정된 공개키로 검증되지 않음 | **아니오** | `signature_invalid` 로 보고하고 받은 파일·서명 파일을 지운다. 다음 poll 에 다시 시도한다 | **단순 실패로 읽지 않는다.** sha256 은 통과했는데 서명만 틀렸다는 것은 "웹 티어가 내려준 해시와 파일은 서로 맞지만 유지보수자의 개인키로 서명된 것이 아니다" 라는 뜻이라, 아래 "웹 티어 침해" 시나리오가 실제로 일어났을 때 **정확히 이 값이 뜬다.** 반복되면 배포 파이프라인·서버 침해부터 확인한다 |
| **문법 검사 실패** — 받은 스크립트가 `bash -n` 을 통과하지 못함 | **아니오** | `syntax_check_failed` 로 보고하고 교체하지 않는다. 다음 poll 에 다시 시도한다 | 서명까지 통과한 파일이 문법에서 걸리면 배포된 정본 자체가 깨진 것이다 — 교체하면 그 노드의 수집이 통째로 멈추므로, 바꾸기 **전에** 거른다 |

#### 전송·교체 단계에서 멈추는 경우

| 상황 | 적용되나 | 동작·보고값 | 왜 그렇게 했나 |
|---|---|---|---|
| **다운로드 실패** — `agent-dl.php` 응답이 비었거나 `curl`/`wget` 이 실패 | **아니오** | `download_failed` 로 보고한다. 다음 poll 에 다시 시도한다(백오프 없음) | 네트워크·서버 일시 장애가 대부분이라 재시도만으로 풀린다. 백오프를 두지 않는 것은 "포기하지 않음" 자체가 목적이기 때문이다 |
| **백업 실패** — 교체 전 `.bak` 복사(`cp -p`)가 실패 | **아니오** | `backup_failed` 로 보고하고 교체하지 않는다 | 롤백할 원본을 못 남기면 자기점검 실패 시 되돌릴 수단이 사라진다. 디스크 가득참·권한 문제가 흔한 원인이라 노드 상태를 봐야 한다 |
