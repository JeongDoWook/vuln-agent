...
`run.sh` 는 그 응답을 보면:
1. 새 스크립트를 받아 **sha256 검증**(불일치 시 적용하지 않고 로그만 남기며 다음 poll 에서
   재시도 — 서버가 여전히 구버전이라 보는 한 자동으로 계속 시도된다).
2. `bash -n` 문법 검사 통과 시 기존 스크립트를 `.bak` 으로 백업하고 `mv` 로 원자적 교체.
3. 다음 실행(정기수집 또는 예약 명령)이 성공하면 `.bak` 을 지우고, **실패하면 `.bak` 으로
   자동 롤백**한다(1회만 — 재귀적으로 계속 시도하지 않는다).
4. 적용 결과(성공/실패/롤백)는 다음 poll GET 에 실어 서버로 보고되고, 서버는 감사로그
